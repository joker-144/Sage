...
- **思考内容输出**：自动捕获推理模型（如 DeepSeek-R1）的思考链（reasoning\_content），以独立卡片展示，默认折叠，支持点击展开查看完整推理过程。
...
- **成稿审阅视图**：写作模式成稿后进入独立审阅工作台，支持章节树导航、字数进度跟踪（章节目标字数 / 当前字数）、AI 痕迹检测、LLM 深度改写、`【数据】`占位回填（内联填写后**直接替换该处占位**，纯字符串替换、不调 LLM）、章节锁定、定向修订、Word 导出与全文引用真实性核查。每次修订 / 改写 / 数据回填前自动存档快照，可随时回滚。多对话各自生成的论文互相隔离（按对话 ID 存储于 `.sage/papers/{对话ID}/`），审阅列表页展示每篇草稿的主题、字数进度与更新时间，可随时返回切换。

- **回复排版自愈**：个别 LLM 会输出逐词换行（竖排）或把 markdown 表格打散成碎片；后端在消息落库前自动规整（竖排碎片合并、散落的表格还原为标准表格），并对存量历史消息提供 `/api/review/fix-vertical-messages` 一键修复。
...
| 数据回填    | 扫描 `【数据】` 占位，内联填写数据后**直接替换该处占位**（纯字符串替换，不走 LLM） |
| 章节锁定    | 锁定指定章节，后续 AI 修改不再触碰（保护已定稿内容）           |
| 定向修订    | 指令式修订指定章节；修订全文若字数缩水超 30% 判定为截断，拒绝覆盖并告警 |
| 版本历史    | 修订 / 改写 / 数据回填前自动存档快照，可查看并一键回滚           |
| Word 导出   | 将当前草稿按大纲顺序导成 `.docx`                     |
| 引用核验    | 对全文 `[CITE:]` 标记与文末条目逐条做 CrossRef/维普/万方硬校验，生成真实性报告 |
| 排版修复    | 修复历史消息的竖排 / 乱版与被打散的表格（`fix-vertical-messages`） |
...
Sage 支持多工作空间，按"时间戳\_领域标签"命名（如 `20260721_143022_CS-AI`），每个工作空间独立 SQLite 索引库，互不污染。
...
| POST | `/api/review/data-fill`                 | `【数据】`占位回填——按节内序号直接替换该处占位，纯替换、不走 LLM（需 `conversation_id`） |
| POST | `/api/review/revise`                    | 定向修订章节（需 `conversation_id`，带保留率校验防缩水）      |
| POST | `/api/review/lock`                      | 锁定/解锁章节（需 `conversation_id`）               |
| GET  | `/api/review/versions`                  | 列出全部版本快照（新→旧，含原因/时间/字数）                  |
| POST | `/api/review/versions/restore`          | 用指定快照覆盖当前草稿并落盘（回滚）                       |
| POST | `/api/review/export-docx`               | 导出当前草稿为 `.docx`（按大纲顺序，跳过无意义章节）           |
| POST | `/api/review/verify-references`         | 引用真实性硬校验（逐条 CrossRef/维普/万方，确定性、不走 LLM）    |
| GET  | `/api/review/citation-report`           | 读取最近一次引用验证报告（不重新联网）                      |
| POST | `/api/review/fix-vertical-messages`     | 修复历史消息竖排/乱版表格（`dry_run=true` 预览不写库）        |
...
| 测试文件                           | 覆盖范围                                                                   |
| ------------------------------ | ---------------------------------------------------------------------- |
| `test_paper_project.py`        | PaperProject 共享草稿（大纲/素材/分节存储、save/load 往返、finalize 落盘、read\_draft 一致性） |
